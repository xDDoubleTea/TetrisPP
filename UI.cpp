...
void UI::update()
{
    ...
    case STATE::PLACE: {
        ...
        int w = al_get_bitmap_width(bitmap);
        int h = al_get_bitmap_height(bitmap);
        Rectangle place_region { mouse.x - w / 2, mouse.y - h / 2,
            DC->mouse.x + w / 2, DC->mouse.y + h / 2 };
        bool place = true;
...
void UI::draw()
{
    ...
    for (auto& [bitmap, p, price] : tower_items) {
        ...
        al_draw_rectangle(p.x - 1, p.y - 1, p.x + w + 1, p.y + h + 1,
            al_map_rgb(0, 0, 0), 1);
        al_draw_textf(FC->courier_new[FontSize::MEDIUM], al_map_rgb(0, 0, 0),
            p.x + w / 2, p.y + h, ALLEGRO_ALIGN_CENTRE, "%d", price);
    }
